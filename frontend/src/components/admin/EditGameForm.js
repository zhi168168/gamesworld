...
const EditGameForm = ({ game, onGameUpdated, onCancel }) => {
      ...
      if (game.image) {
        setImagePreview(
          game.image.startsWith('http') 
            ? game.image 
            : `${process.env.REACT_APP_API_URL || ''}${game.image}`
        );
      }
    }
  }, [game]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`/api/dalianmao/games/${game._id}`, formPayload, {
        headers: {
      ...
      setError(error.response?.data?.message || 'Failed to update game. Please try again.');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="card bg-dark mb-4">
      <div className="card-header d-flex justify-content-between align-items-center">
        <h4 className="mb-0">编辑游戏: {game.name}</h4>
          ...
          <div className="row">
            ...
            <div className="col-md-6">
              ...
              <div className="mb-3">
                ...
                />
                <small className="text-muted">留空则保留当前图片。支持的格式：JPG, PNG, WebP（最大5MB）</small>
              </div>
              ...
              <div className="mt-3">
                <p className="text-muted">游戏ID: {game.gameId} (不可更改)</p>
              </div>
            </div>
          </div>
